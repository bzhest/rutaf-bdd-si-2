...
Feature: Questionnaire Management - Question Tab - Single Select Question Type
  ...
  @C46739572
  Scenario: C46739572: [Questionnaire] - Add/Edit Question - Single Select - Responder tab
    Then Questionnaire setup "Tab One" tab is selected
    When User adds question with type "SingleSelect" to active tab
    Then Questionnaire "SingleSelect" question on position 1 is displayed with default elements
    When User clicks Configuration icon
    Then Questionnaire Main question section is greyed out
    And Questionnaire "SingleSelect" configuration section appears with expected elements for "Responder use"
    ...
    Then Questionnaire question configuration "See less" button is displayed
    And Questionnaire choices with label "Choice #" 4 options are displayed
    When User configures question with data "SingleSelect question configuration with 5 choices"
    ...
    Then Question configuration contains expected data "SingleSelect question configuration with 5 choices"
    When User configures question with data "SingleSelect question configuration with 6 choices"
    ...
    And User clicks "Question" questionnaire tab
    And User clicks Configuration icon
    Then Question configuration contains expected data "SingleSelect question configuration with 6 choices"

  @C46739574
  Scenario: C46739574: [Questionnaire] - Add/Edit Question - Single Select - Reviewer tab - changed number of reviewers
    When User adds question with type "SingleSelect" to active tab
    ...
    When User clicks Configuration icon
    Then Questionnaire Main question section is greyed out
    And Questionnaire "SingleSelect" configuration section appears with expected elements for "Reviewer use"
    When User clicks Add button for question "Choice #2" choice
    Then Questionnaire choices with label "Choice #" 3 options are displayed
    And Questionnaire choices "Choice #" Delete buttons are displayed
    And Questionnaire choices counter is "Choices: 3/250"
    ...
    Then Questionnaire question configuration "See less" button is displayed
    And Questionnaire choices with label "Choice #" 4 options are displayed
    When User configures question with data "SingleSelect question configuration"
    And User clicks Configuration icon
    ...
    Then Question Reviewer level drop-down options are
      ...
      | Level 4 |
      | Level 5 |
    When User configures question with data "SingleSelect question configuration with Level 2 reviewer"
    ...
    And User clicks on Question tab with index 1
    And User clicks Configuration icon
    Then Question configuration contains expected data "SingleSelect question configuration with Level 2 reviewer"

  @C46739599
  Scenario: C46739599: [Questionnaire] - Add/Edit Question - Single Select - Delete
    When User adds question with type "SingleSelect" to active tab
    And User clicks Configuration icon
    And User configures question with data "SingleSelect question configuration with 5 choices"
    And User clicks Configuration icon
  ...
  @C46739593
  Scenario Outline: C46739593: [Questionnaire] - Add/Edit Question - Single Select - Bulk Choices - Add
    When User adds question with type "SingleSelect" to active tab
    And User clicks Add question tab button
    ...
    And Questionnaire "SingleSelect" configuration section appears with expected elements for "<tabType>"
    When User clicks Questionnaire Setup button "Add Bulk Choices"
    Then Questionnaire Add Bulk 250 Choices modal is displayed with default elements
    When User fills in Bulk Choices values
      | \|1 |
    ...
    Then Questionnaire Add Bulk Choices modal is not displayed
    And Question configuration Choices contain expected data "SingleSelect question configuration with 5 choices bulk added"
  ...
  @C46739593
  Scenario Outline: C46739593: [Questionnaire] - Add/Edit Question - Single Select - Bulk Choices - Edit
    When User adds question with type "SingleSelect" to active tab
    And User clicks Questionnaire Setup button "Next"
    And User clicks Questionnaire Setup button "<button>"
    And User clicks edit added questionnaire
  ...
  @C46739673
  Scenario Outline: C46739673: [Questionnaire] - Add/Edit Question - Single Select - Mapping
    When User creates Custom Field "active Single Select type" via API
    ...
    Then Question Mapping section is displayed with default data for "SingleSelect" question type
