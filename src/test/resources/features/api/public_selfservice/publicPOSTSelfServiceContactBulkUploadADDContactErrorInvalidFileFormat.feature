@api @public_api @postPublicSelfServiceContactBulkUploadADDContactErrorInvalidFileFormat
...
#    And print "******** CONTACTSHEADERS  = '"+ contactsTemplateHeaders +"'"
    ...
    * def nameVarTARGET =       'ADD-3P-ERR-INVFLFRMT'
...
## #####################################################################################################################
## 1) INVALID FILE FORMAT: Wrong - additional column header
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorInvalidFileFormat_addlHdr.csv' )
...
### #####################################################################################################################
### 2) INVALID FILE FORMAT: Wrong - comma-delimited
#
##
## ADD CONTACT: by SELF-SERVICE ThirdParty Contact Bulk Upload
##
...
##    * def csvRowsT = csvRows00 + csvRows01
#
#    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorInvalidFileFormat_commaDltd.csv' )
#    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
#    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
#    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
#
