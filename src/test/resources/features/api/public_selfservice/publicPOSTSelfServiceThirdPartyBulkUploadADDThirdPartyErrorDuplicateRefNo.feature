@api @public_api @postPublicSelfServiceThirdPartyBulkUploadADDThirdPartyErrorDuplicateRefNo
...
#      """
  ...
  Scenario: 48) C37786660 Third-party - ADD
    ...
    * def nowMs = call fnGetNowMs
    ...
    And print '\n'
    And print '================================'
    And print '******** SELF-SERVICE THIRD-PARTY: SCENARIO 01) ADD DUPLICATE REF. NO. ERROR'
    And print '******** nowMs = '+ nowMs
    And print '================================\n'
    And print '\n'

    * def nameVarTARGET =       'ADD-3P-DUPLREFNO-ERR'
...
#   * def csvRows01 = "FULLUPDATE |"+    thirdPartyId    +"|"+    nowMs   +"|1       |FULLUPDATE - "+ nowMs    +"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987            |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank|12 Finance Avenue|12345678   |Reno     |MB Reno Br.|United States|12 Main Ave.|Ontario       |Ottawa|Americas|Z1234          |Canada     |+18001      |www.test1.com|123|test1@test.ph|Desc abc123|                    |Acme Inc. |Local Language Name|France            |Field1            |FULLUPDATE TEXT    \n"
    * def csvRows01 = "ADD        |                        |"+    nowMs   +"|1       |"+nameVarTARGET+"-"+nowMs+"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987           |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank01|11 Finance Avenue|11111111   |Reno     |MB Reno Br.|United States|11 Main Ave.|Ontario       |Ottawa|Americas|ZIP90210       |Canada       |+01800111111|www.test01.com|111|test01@test.com |Test desc01|                    |Acme01 Corp.|Local Language Name|France            |Field1                     |Test CUSTOM FLD VAL01|              |                         |           |                |\n"
