@api @public_api @postPublicSelfServiceThirdPartyBulkUploadADDThirdParty5ContactInfosSuccess
...
#      """
  ...
  Scenario: 43) C37786687 Third-party - ADD
    ...
    * def nowMs = call fnGetNowMs
    ...
    And print '\n'
    And print '================================'
    And print '******** SELF-SERVICE THIRD-PARTY: SCENARIO 01) ADD 5 CONTACT INFOS SUCCESS'
    And print '******** nowMs = '+ nowMs
    And print '================================\n'
    And print '\n'

    * def nameVarTARGET =       'ADD-3P-5CONTACTINFOS-SUCC'
...
#   * def csvRows01 = "FULLUPDATE |"+    thirdPartyId    +"|"+    nowMs   +"|1       |FULLUPDATE - "+ nowMs    +"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987            |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank01|12 Finance Avenue|12345678   |Reno     |MB Reno Br.|United States|12 Main Ave.|Ontario       |Ottawa|Americas|Z1234          |Canada       |+18001      |www.test1.com|111|email01@test.com |Desc abc123|                    |Acme Inc. |Local Language Name|France                  |Field1            |FULLUPDATE TEXT    \n"
    * def csvRows01 = "ADD        |                        |"+    nowMs   +"|C01     |"+nameVarTARGET+"-"+nowMs+"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987           |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank01|01 Finance Avenue|11111111   |Reno     |MB Reno Br.|United States|12 Main Ave.|Ontario       |Ottawa|Americas|ZIP90210       |Canada       |180011111111|www.test01.com|111|email01@test.com|Test desc. |                    |Acme Corp.|Local Language Name|France                  |Field1            |Test Field1 VALUE  |              |                         |           |                |\n"
    * def csvRows02 = "           |                        |                |C01     |                           |            |        |                 |             |                 |                     |       |                |           |                    |          |              |              |               |                |             |                       |                    |              |           |                 |           |         |           |             |            |              |      |        |               |             |180022222222|www.test02.com|222|email02@test.com|           |                    |          |                   |                        |                  |                   |              |                         |           |                |\n"
    * def csvRows03 = "           |                        |                |C01     |                           |            |        |                 |             |                 |                     |       |                |           |                    |          |              |              |               |                |             |                       |                    |              |           |                 |           |         |           |             |            |              |      |        |               |             |180033333333|www.test03.com|333|email03@test.com|           |                    |          |                   |                        |                  |                   |              |                         |           |                |\n"
    * def csvRows04 = "           |                        |                |C01     |                           |            |        |                 |             |                 |                     |       |                |           |                    |          |              |              |               |                |             |                       |                    |              |           |                 |           |         |           |             |            |              |      |        |               |             |180044444444|www.test04.com|444|email04@test.com|           |                    |          |                   |                        |                  |                   |              |                         |           |                |\n"
    * def csvRows05 = "           |                        |                |C01     |                           |            |        |                 |             |                 |                     |       |                |           |                    |          |              |              |               |                |             |                       |                    |              |           |                 |           |         |           |             |            |              |      |        |               |             |180055555555|www.test05.com|555|email05@test.com|           |                    |          |                   |                        |                  |                   |              |                         |           |                |\n"
...
#    * def customField_Field1 = $retrieveThirdPartyByIdResult1[0].response.data.customFields[ ?(@.name=='Field1') ]
    * def phoneNumbers_LENGTH =   retrieveThirdPartyByIdResult1[0].response.data.contactInformation.phoneNumber.length
    And match phoneNumbers_LENGTH == 5

    And print '\n'
    And print '======== VALIDATE ================================'
    And print 'phoneNumbers_LENGTH = "'+ phoneNumbers_LENGTH +'"'
    And print '==================================================\n'
    And print '\n'
