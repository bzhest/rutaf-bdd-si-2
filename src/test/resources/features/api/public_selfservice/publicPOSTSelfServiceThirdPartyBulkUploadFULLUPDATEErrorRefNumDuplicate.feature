@api @public_api @postPublicSelfServiceThirdPartyBulkUploadFULLUPDATEErrorRefNumDuplicate
...
# Success: reference no.
    * def csvRows01 = "FULLUPDATE |"+    thirdPartyId    +"|"+   nowMs2   +"|1       |FULUPD"+nowMs2+"|            |        |                 |             |                 |                     |       |                |           |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |              |               |                |             |                       |              |              |         |                 |           |         |           |            |            |              |    |      |               |Philippines|            |       |   |             |           |                    |          |               |                  |                  |                   |              |                         |           |                |\n"
