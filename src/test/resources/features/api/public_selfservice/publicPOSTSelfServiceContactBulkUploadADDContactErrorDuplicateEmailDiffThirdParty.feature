@api @public_api @postPublicSelfServiceContactBulkUploadADDContactErrorDuplicateEmailDiffThirdParty
...
#    And print "******** CONTACTSHEADERS  = '"+ contactsTemplateHeaders +"'"
    ...
    * def nameVarTARGET =       'ADD-3P-DUPLEMAILDIFF3P-ERR'
...
#    * def csvRows02 = "ADD        |                        |"+    nowMs   +"|2       |"+nameVarTARGET+"-"+nowMs+"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987           |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank|12 Finance Avenue|12345678   |Reno     |MB Reno Br.|United States|12 Main Ave.|Ontario       |Ottawa|Americas|ZIP90210       |Canada     |0018001     |www.test1.com|234|tst@email.com|Test desc. |                    |Acme Corp.|Local Language Name|France            |Field1            |Test CUSTOM FLD VAL|              |                         |\n"
    * def csvRows02 = "ADD        |                        |                |2       |"+nameVarTARGET+"-"+nowMs+"|            |        |                 |             |                 |                     |       |                |           |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |              |               |                |             |                       |                    |              |         |                 |           |         |           |             |            |              |      |        |               |Italy        |            |             |   |             |           |                    |          |                   |                  |                  |                   |              |                         |           |                |\n"
...
#
    ...
    * def validateSuccess1 = ( retrieveThirdPartySelfServiceProcess1_response.data.recordProcessed==2 && retrieveThirdPartySelfServiceProcess1_response.data.recordWithErrors==0 )
...
#    And print "retrieveThirdPartyListResult0 = \n", retrieveThirdPartyListResult0
    ...
    * def thirdPartyId1 = thirdPartyIdsTODAYTARGETArr[0].thirdPartyId
    * def thirdPartyId2 = thirdPartyIdsTODAYTARGETArr[1].thirdPartyId
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorDuplicateEmailDiffThirdParty.csv' )
    * def csvAsString = replacePlaceholderString( csvAsString, '[THIRD-PARTY-ID1]',   ''+thirdPartyId1 )
    * def csvAsString = replacePlaceholderString( csvAsString, '[THIRD-PARTY-ID2]',   ''+thirdPartyId2 )
    ...
    * def csvRowsT =    replacePlaceholderString( csvAsString, '[CONTACT-EMAIL-MS]', ''+(nowMs     ) )
