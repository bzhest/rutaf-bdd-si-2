@api @public_api @postPublicSelfServiceThirdPartyBulkUploadFULLUPDATEErrorInvalidActionType
...
# Error: Action Type = Invalid
    * def csvRows01 = "XXXXXXXX   |"+    thirdPartyId    +"|"+    nowMs   +"|1       |FULUPD-ACTTYP -"+nowMs+"|            |        |                 |             |                 |                     |       |                |           |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |              |               |                |             |                       |              |              |         |                 |           |         |           |            |            |              |    |      |               |Canada     |            |           |   |             |           |                    |          |               |                  |                           |                    |              |                         |           |                |\n"
