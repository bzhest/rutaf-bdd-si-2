@api @public_api @postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields
...
#      """
    ...
    * def replaceThirdPartyIdPlaceholder =
      """
        function( targetString, placeholder, thirdPartyId ) {

          return targetString.replace( placeholder, thirdPartyId );
        }
      """
...
#    And print "******** CONTACTSHEADERS  = '"+ contactsTemplateHeaders +"'"
    ...
    * def nameVarTARGET =       'ADD-3P-MISSREQDFLDS-ERR'
...
### #####################################################################################################################
## 1) MISSING REQUIRED FIELDS - 'Action Type'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_actTyp.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 2) MISSING REQUIRED FIELDS - 'Third-party ID'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_3pId.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 3) MISSING REQUIRED FIELDS - 'Group ID'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_grpId.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 4) MISSING REQUIRED FIELDS - 'Contact First Name'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_nameFirst.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 5) MISSING REQUIRED FIELDS - 'Contact Last name'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_nameLast.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 6) MISSING REQUIRED FIELDS - 'Email Address'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_email.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
...
### #####################################################################################################################
## 7) MISSING REQUIRED FIELDS - 'Auto Screen'
...
#    * def csvRowsT = csvRows00 + csvRows01
    ...
    * def csvAsString = karate.readAsString( './data/postPublicSelfServiceContactBulkUploadADDContactErrorMissingReqdFields_autoScreen.csv' )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[THIRD-PARTY-ID]',   ''+thirdPartyId )
    * def csvAsString = replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-REF-ID]',   ''+(nowMs+1000) )
    * def csvRowsT =    replaceThirdPartyIdPlaceholder( csvAsString, '[CONTACT-EMAIL-MS]', ''+nowMs )
