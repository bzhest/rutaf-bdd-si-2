@api @public_api @postPublicSelfServiceThirdPartyBulkUploadADDThirdPartyErrorInvalidSegmentationValues
...
#      """
  ...
  Scenario: 102) C37786662 Third-party - ADD
    ...
    * def nowMs = call fnGetNowMs
    ...
    And print '\n'
    And print '================================'
    And print '******** SELF-SERVICE THIRD-PARTY: SCENARIO 01) ADD INVALID SEGMENTATION VALUES ERROR'
    And print '******** nowMs = '+ nowMs
    And print '================================\n'
    And print '\n'

    * def nameVarTARGET =       'ADD-3P-INVSEGMVAL-ERR'
...
#    * def csvRows01 = "ADDXXXXX   |                        |"+    nowMs   +"|1       |"+nameVarTARGET+"-"+nowMs+"|CORP        |USD     |5K-10K           |AHI          |PRS              |02/22/1987           |10M    |11/02/2008      |SOE        |rddcentre.admin.np@refinitiv.com|MyDivision|Default       |OTS           |DIST_M_SRC     |OTS             |MULTI        |LITTLE_VIS             |COMMODITIZED_PRODUCT|A3P           |MoneyBank|12 Finance Avenue|12345678   |Reno     |MB Reno Br.|United States|12 Main Ave.|Ontario       |Ottawa|Americas|ZIP90210       |Canada     |0018001     |www.test1.com|234|tst@email.com|Test desc. |                    |Acme Corp.|Local Language Name|France            |Field1            |Test CUSTOM FLD VAL\n"
#   * def csvRows01 = "ADD|||1|"+nameVarTARGET+"-"+nowMs+"||||||||||rddcentre.admin.np@refinitiv.com|MyDivision|Default|dggg|gggg|gggg|gggg|gggg|ggg|gggg||||||||||||Philippines|||||||||||\n"
    * def csvRows01 = "ADD         |                       |                |1       |"+nameVarTARGET+"-"+nowMs+"|           |         |                |              |                 |                      |      |                |            |rddcentre.admin.np@refinitiv.com|MyDivision|Default      |XXXXXXXXXXXXXX|XXXXXXXXXXXXXXX|XXXXXXXXXXXXXXXX|XXXXXXXXXXXXX|XXXXXXXXXXXXXXXXXXXXXXX|XXXXXXXXXXXXXXXXXXXXX|XXXXXXXXXXXXXX|         |                 |           |         |           |             |            |              |       |       |               |Philippines|             |             |   |             |          |                    |           |                  |                   |                |                   |              |                         |           |                |\n"
