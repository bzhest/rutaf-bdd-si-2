@api @public_api @deletePublicThirdPartyDeleteThirdParty
Feature: Delete Third Party
...
#      """
  ...
  Scenario: 1) C36276926 Public API - DELETE/thirdparty/{id}
               - Verify that third party is deleted if status is New
...
##
    ...
    * def incDateResp =       response.data.incorporationDate
    ...
    And print '\n'
    And print '================================'
    And print '******** DELETE THIRD-PARTY: CREATE THIRD-PARTY'
...
#
    ...
    * def eTag = responseHeaders['ETag'][0].split('"')[1]
    And print '\n'
    And print '================================'
    And print '******** DELETE THIRD-PARTY: RETRIEVE THIRD-PARTY BY ID'
...
#
    ...
    * def count = response.data.length
    And print '\n'
    And print '================================'
    And print '******** DELETE THIRD-PARTY: RETRIEVE THIRD-PARTY LIST'
...
#
  ...
  Scenario: 2) C36276928 Public API - DELETE/thirdparty/{id}
               - Verify error if supplier Id does not exist

#
# DELETE third-party by id - INVALID id
#
    * def nowMs = call fnGetNowMs

    Given url deleteUrl = baseTestUrl
    And path 'thirdparty', nowMs
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    When method DELETE
    Then status 404
...
#
    And match response ==
          """
              {
                "message": 'Third-party doesn’t exist.'
              }
          """
