@api @public_api @getPublicThirdPartyRetrieveThirdPartyWithStatusChangeList
Feature: Get Third Party With Status Change List
...
#    And match incDateResp ==  '#regex ^[0-9]{4}-[0-9]{2}-[0-9]{2}[T][0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]+)?(Z|[+-][0-9]{2}:[0-9]{2}|[+-][0-9]{4})$'
  ...
  Scenario: 1) C36505206 RMS-5621 SIConnect Public API V2
               - Get Supplier list with Change of Status by Update Date Range

#
# Get Third Party With STATUS Change List
#
...
#    And param start =          nowStartISODate4
    ...
    * def nextPageToken = response.meta.nextPageToken
    And print '\n'
    And print '================================'
    And print '******** RETRIEVE THIRD-PARTY WITH STATUS CHANGE LIST'
  ...
  Scenario: 2) C36505214 RMS-5621 SIConnect Public API V2
               - Get Supplier list with Change of Status by Update Date Range - Invalid date range
...
#
    ...
    And param end =            now365AgoISODate4
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    When method GET
    Then status 400
...
#
    ...
    And path 'thirdparty', 'monitor', 'status'
    And param pageSize =       100
    And param start =          'ABC'+ now365AgoISODate4
  ...
  Scenario: 3) C36505299 RMS-5621 SIConnect Public API V2
               - Get Supplier list with Change of Status by Update Date Range
               - pagesize is null
...
#    And param pageSize =       100
  ...
  Scenario: 4) C36509366 RMS-5621 SIConnect Public API V2
               - Get Supplier list with Change of Status by Update Date Range
