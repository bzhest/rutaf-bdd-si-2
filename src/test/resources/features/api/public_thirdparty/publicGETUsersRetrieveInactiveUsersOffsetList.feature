@api @public_api @getPublicUsersRetrieveInactiveUsersOffsetList
Feature: Retrieve Users Offset-List
...
#      """
  ...
  Scenario:
    ...
    * def search =         'Thomas'
    * def fetchType =      'INACTIVE'
    * def page =           0
    * def pageSize =       5
#    * def requestorEmail = requestorEmail
#    * def xTenantCode =    xTenantCode

    * def retrieveUsersOffsetListResult = call read( "./call/getUsersOffsetList.feature" )   { "search": "#(search)", "fetchType": "#(fetchType)", "page": "#(page)", "pageSize": "#(pageSize)", "requestorEmail": "#(requestorEmail)", "xTenantCode": "#(xTenantCode)" }

#    * print "################################ retrieveThirdPartyOffsetListResult.response = \n", retrieveThirdPartyOffsetListResult.response
    * def dataLength = retrieveUsersOffsetListResult.response.data.length
