@api @public_api @getPublicThirdPartyRetrieveThirdPartyById
...
#      """
  ...
  Scenario: 1) C36187805 Public API - GET /thirdparty/{id}
            - Verify that user can retrieve Third Party Details by Supplier ID
...
#
# Get sample suppliers from which to select a third-party id
#
    ...
    * def thirdPartyId = response.data[ response.data.length - 1 ].id
    And print '\n'
    And print '================================'
    And print '******** RETRIEVE THIRD-PARTY BY ID: RETRIEVE THIRD-PARTY LIST'
...
#
  ...
  Scenario: 2) C36187806 Public API - GET /thirdparty/{id}
               - Verify that user can retrieve Third Party details by Reference No.
...
#
    ...
    * def thirdPartyIdResult = response.data.id
    * def referenceNoResult =  response.data.referenceNo



#
# RETRIEVE third-party BY REFERENCE NUMBER
#
    Given url getUrl = baseTestUrl
    And path 'thirdparty', referenceNoResult
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    When method GET
    Then status 200

    * def thirdPartyIdResultFromRefNo = response.data.id

    And match thirdPartyIdResult == thirdPartyIdResultFromRefNo
    ...
    * def eTag = responseHeaders['ETag'][0].split('"')[1]
    And print '\n'
    And print '================================'
    And print '******** RETRIEVE THIRD-PARTY BY ID: RETRIEVE THIRD-PARTY BY REFERENCE NUMBER'
    And print '******** thirdPartyIdResult          = '+ thirdPartyIdResult
    And print '******** thirdPartyIdResultFromRefNo = '+ thirdPartyIdResultFromRefNo
    And print '******** ETag                        = '+ eTag
    And print '================================\n'
    And print '\n'
...
#
  ...
  Scenario: 3) C36187807 Public API - GET /thirdparty/{id}
               - Verify that Response Code is 404 if invalid supplierId is populated in id parameter

#
# Get Third-Party By Id - INVALID id
#


#
# RETRIEVE third-party by id - INVALID id
#
    Given url getUrl = baseTestUrl
    And path 'thirdparty', 'xxxxxxxx'
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    When method GET
    Then status 404
  ...
  Scenario: 4) C36187808 Public API - GET /thirdparty/{id}
               - Verify that Response Code is 404 if invalid Reference No. is populated in id parameter

#
# Get Third-Party By Id - INVALID REFERENCE NO.
#


#
# RETRIEVE third-party by id - INVALID REFERENCE NO.
#
    Given url getUrl = baseTestUrl
    And path 'thirdparty', 'xxxxxxxx'
    And header X-Tenant-Code = xTenantCode
  ...
  Scenario: 5) C36187809 Public API - GET /thirdparty/{id}
...
#
# RETRIEVE third-party BY ID and FIELDS
#
    Given url getUrl = baseTestUrl
    And path 'thirdparty', thirdPartyId
    And param fields = 'data.name, data.address, data.spendCategory'
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    When method GET
    Then status 200
...
#    * def thirdPartyIdResult = response.data.id
    ...
    * def eTag = responseHeaders['ETag'][0].split('"')[1]
    ...
    And print '================================'
    And print '******** RETRIEVE THIRD-PARTY BY ID'
    And print '******** thirdPartyId          = '+ thirdPartyIdResult
    And print '******** ETag                  = '+ eTag
    And print '================================\n'
    And print '\n'
...
#
# VALIDATE response SCHEMA with specified FIELDS
#
