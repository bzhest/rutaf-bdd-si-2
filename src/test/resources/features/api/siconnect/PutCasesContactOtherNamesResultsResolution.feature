@api @siconnect_api
Feature: Resolving case results for contact's other name

  Background:
    * def putUrl = baseUrl + version + '/cases/contact/' + staticContactId + '/otherNames/' + contactOtherName + '/results/resolution'
   #    Retrieve resultIds for other name screening results
    * def getScreeningResults = baseUrl + version + '/cases/contact/' + staticContactId + '/' + contactOtherName + '/otherName'
...
#    Retrieve resolution toolkit to build put request
  ...
  Scenario Outline: Check ability to resolve results for contact's other name
...
#    And contact's screening status updated
    Given header X-Tenant-Code = tenant
    And url getScreeningResults
    And method GET
    And status 200
  ...
  Scenario: Check ability to get error when contact ID is invalid
    ...
    * def invalidContactIdUrl = baseUrl + version + '/cases/contact/' + invalidContactId + '/otherNames/' + contactOtherName + '/results/resolution'
    * def statusId = karate.jsonPath(resolutionToolkit, "$..statuses[?(@.type==\'POSITIVE\')].id")
    * def riskId = karate.jsonPath(resolutionToolkit, "$..resolutionRules." + statusId + ".risks[0]")
    * def reasonId = karate.jsonPath(resolutionToolkit, "$..resolutionRules." + statusId + ".reasons[0]")
    And set requestBody.statusId = statusId[0]
  ...
  Scenario: Check ability to get error when other name is invalid
    * def errorResolve = karate.read('classpath:src/test/resources/testdata/api/error/contactOtherNameResolveFailed.json')
    * def invalidContactIdUrl = baseUrl + version + '/cases/contact/' + staticContactId + '/otherNames/' + contactOtherName + '_invalid/results/resolution'
    * def statusId = karate.jsonPath(resolutionToolkit, "$..statuses[?(@.type==\'POSITIVE\')].id")
    * def riskId = karate.jsonPath(resolutionToolkit, "$..resolutionRules." + statusId + ".risks[0]")
    * def reasonId = karate.jsonPath(resolutionToolkit, "$..resolutionRules." + statusId + ".reasons[0]")
    And set requestBody.statusId = statusId[0]
