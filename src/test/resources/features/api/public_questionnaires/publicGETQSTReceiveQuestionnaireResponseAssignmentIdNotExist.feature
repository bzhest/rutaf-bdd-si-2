@api @public_api @postPublicQSTReceiveQuestionnaireResponseAssignmentIdNotExist
Feature: Receive Questionnaire Response Assignment Id Doest Not Exist
  ...
  Scenario: C40181689 : [QST API] Receive Questionnaire Response - Questionnaire Assignment ID doesn't exist
  ...
  * def language = get response.data.questionnaires[0].languages[0]
    ...
    * def invalidAssignmentId = 0

    Given url postUrl = baseTestUrl
    And path 'thirdparty', thirdPartyId, 'questionnaires', invalidAssignmentId
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And param language = language
