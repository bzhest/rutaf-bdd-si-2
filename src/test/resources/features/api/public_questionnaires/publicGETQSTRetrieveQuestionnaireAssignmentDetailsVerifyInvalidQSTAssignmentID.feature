@api @public_api @getPublicQSTRetrieveQuestionnaireAssignmentDetailsVerifyInvalidQSTAssignmentID
Feature: Get Questionnaire Assignment Details
  ...
  Scenario: C39215947 : Public API - GET/thirdparty/{id}questionnaires/{assignmentId} - Verify error message for Questionnaire Assignment ID that does not exist
  ...
  * def language = get response.data.questionnaires[0].languages[0]
    ...
    Given url getUrl = baseTestUrl
    And path 'thirdparty', thirdPartyId, 'questionnaires', 'invalid'
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
