@api @getRetrieveAssignedActivities @siconnect_api
Feature: Get Retrieve Assigned Activities
  ...
  Scenario: Get Retrieve Assigned Activities

    Given url getUrl = baseTestUrl +'/assigned-activities'
    ...
    And param startDate = '946656000000'
    And param endDate = '1623828495388'
    When method GET
    Then status 200
#    And print 'RESPONSE: \n', response
...
#
    ...
    * if ( totalPages <= 1 ) karate.abort()


#
# REPEAT call to api endpoint for LAST page
#
    Given header X-Tenant-Code = 'qa-sprint2'
    And header X-trace-id = 'x'
    And header requestor = 'annacarissa.valerio@refinitiv.com'
    And header requestorEmail = 'annacarissa.valerio@refinitiv.com'
    And param currentPage = totalPages
    And param limit = 10
    And param email = 'annacarissa.valerio@refinitiv.com'
    And param startDate = '946656000000'
    And param endDate = '1623828495388'
