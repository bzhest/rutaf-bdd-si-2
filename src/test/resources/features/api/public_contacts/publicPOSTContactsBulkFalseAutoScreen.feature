@api @public_api @postContactBulkFalseAutoScreen
Feature: Post Contacts Bulk
  ...
  Scenario: C36333413 : Public API - POST /contacts/bulk - Verify that Created Contacts are not screened against WC if auto screen is false
...
#
    ...
    * def newPostRequestBodyBulkContactsFalseAutoScreen =
        """
          [
            {
              "parent": "#(thirdPartyId)",
    ...
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And request newPostRequestBodyBulkContactsFalseAutoScreen
    When method POST
    Then status 207
