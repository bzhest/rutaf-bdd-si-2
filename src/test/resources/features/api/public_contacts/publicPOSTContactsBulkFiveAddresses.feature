@api @public_api @postContactBulkFiveAddresses
Feature: Post Contacts Bulk
  ...
  Scenario: C36333412 : Public API - POST /contacts/bulk - Verify that contacts with five addresses are successfully created
...
#
    ...
    * def newPostRequestBodyForFiveAddresses =
        """
          [
            {
              "parent": "#(thirdPartyId)",
    ...
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And request newPostRequestBodyForFiveAddresses
    When method POST
    Then status 207
