@api @public_api @postContactBulkErrorRequireFields
Feature: Post Contacts Bulk
  ...
  Scenario: C36333432 : Public API - POST /contacts/bulk - Verify error for required fields
...
#
    ...
    * def newPostRequestBodyErrorRequireFields =
        """
          [
            {
              "parent": "#(thirdPartyId)",
    ...
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And request newPostRequestBodyErrorRequireFields
    When method POST
    Then status 207
