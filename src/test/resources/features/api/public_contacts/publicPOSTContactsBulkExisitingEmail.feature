@api @public_api @postContactBulkExistingEmail
...
#      """
  ...
  Scenario: C36333417 : Public API - POST /contacts/bulk - Verify error for active contact with existing email
...
#
# Create BULK CONTACT - DUPLICATE EMAIL; "isActive": true
#

    * def newPostRequestBodyForExistingEmail =
        """
          [
            {
              "parent": "#(thirdPartyId)",
    ...
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And request newPostRequestBodyForExistingEmail
    When method POST
    Then status 207
