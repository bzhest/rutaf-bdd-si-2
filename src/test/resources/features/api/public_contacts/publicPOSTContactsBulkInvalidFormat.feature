@api @public_api @postContactBulkInvalidFormat
Feature: Post Contacts Bulk
  ...
  Scenario: C36333418 : Public API - POST /contacts/bulk - Verify error for invalid format
...
#
    ...
    * def newPostRequestBodyForInvalidFormat =
        """
          [
            {
              "parent": "#(thirdPartyId)",
    ...
    And header X-Tenant-Code = xTenantCode
    And header Authorization = 'Bearer '+ accessToken
    And request newPostRequestBodyForInvalidFormat
    When method POST
    Then status 207
    ...
    And match response.data[0].errors ==
     """
      [
        {
          "code":        'SI-ERR-003',
          "field":       'contactInformation.email[0].<list element>',
          "description": '##regex ^.*[(is not valid.)]$'
        }
      ]
    """
